...
public final class Account {
	...
	public static void main(String[] args) {
		System.out.println(Long.parseUnsignedLong("9037328626462718729"));
		System.out.println(Long.parseUnsignedLong("6163667937706295802"));
		System.out.println(Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS,Long.parseUnsignedLong("9037328626462718729")));
		System.out.println(Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS,Long.parseUnsignedLong("6163667937706295802")));
		
		// 张巍
		// String publicKey =
		// "e115049da84efb677d48caf10628bbe4e732fe53715c14aaccf70c8499e30d03";

		// CREATOR_PUBLIC_KEY
		System.out.println(getId(Genesis.CREATOR_PUBLIC_KEY));

		String CREATOR_PUBLIC_KEY = "11858cb9a3f24f3c7de187848362746ce8abf58ac85889616182edbcf8701002";
		byte[] aa = CREATOR_PUBLIC_KEY.getBytes();
		System.out.println("length=" + aa.length);
		/*
		 * for (int i = 0; i < aa.length; i++) { System.out.println(aa[i]); }
		 */

		//用户 根据密码获取公钥，再根据公钥获取accountId
		byte[] PublicKeyByte = Crypto
				.getPublicKey("spell nation scratch satisfy watch closet normal color heat due recall movie");
		Long accountId = getId(PublicKeyByte);
		
		// 
		System.out.println("accountId=" + accountId);
		System.out.println("zwAccountId="+getId(Crypto.getPublicKey("end apart laid tough handle spirit tease random fought adore truck stress")));
	}
	...
	public static final class AccountAsset {
		...
		public String toString() {
			return "AccountAsset account_id: " + Long.toUnsignedString(accountId) + " asset_id: "
					+ Long.toUnsignedString(assetId) + " quantity: " + quantityQNT + " unconfirmedQuantity: "
	...
	public static final class AccountCurrency {
		...
		public String toString() {
			return "AccountCurrency account_id: " + Long.toUnsignedString(accountId) + " currency_id: "
					+ Long.toUnsignedString(currencyId) + " quantity: " + units + " unconfirmedQuantity: "
					+ unconfirmedUnits;
		}
	...
	public static final class AccountProperty {
		...
		private void save(Connection con) throws SQLException {
			try (PreparedStatement pstmt = con.prepareStatement(
					"MERGE INTO account_property " + "(id, recipient_id, setter_id, property, value, height, latest) "
	...
	private static final DbKey.LinkKeyFactory<AccountAsset> accountAssetDbKeyFactory = new DbKey.LinkKeyFactory<AccountAsset>(
			"account_id", "asset_id") {
	...
	private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>(
			"account_asset", accountAssetDbKeyFactory) {
		...
		public void checkAvailable(int height) {
			if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Nxt.getBlockchainProcessor()
	...
	private static final DbKey.LinkKeyFactory<AccountCurrency> accountCurrencyDbKeyFactory = new DbKey.LinkKeyFactory<AccountCurrency>(
	...
	private static final VersionedEntityDbTable<AccountCurrency> accountCurrencyTable = new VersionedEntityDbTable<AccountCurrency>(
			"account_currency", accountCurrencyDbKeyFactory) {
	...
	private static final DbKey.LongKeyFactory<AccountProperty> accountPropertyDbKeyFactory = new DbKey.LongKeyFactory<AccountProperty>(
			"id") {
	...
	private static final VersionedEntityDbTable<AccountProperty> accountPropertyTable = new VersionedEntityDbTable<AccountProperty>(
			"account_property", accountPropertyDbKeyFactory) {
	...
	public static long getCurrencyUnits(long accountId, long currencyId, int height) {
		AccountCurrency accountCurrency = accountCurrencyTable
				.get(accountCurrencyDbKeyFactory.newKey(accountId, currencyId), height);
		return accountCurrency == null ? 0 : accountCurrency.units;
	}

	public static long getCurrencyUnits(long accountId, long currencyId) {
		AccountCurrency accountCurrency = accountCurrencyTable
				.get(accountCurrencyDbKeyFactory.newKey(accountId, currencyId));
		return accountCurrency == null ? 0 : accountCurrency.units;
	}
			...
			Nxt.getBlockchainProcessor().addListener(block -> {
				...
				block.getTransactions().forEach(transaction -> {
					...
					if (transaction.getType() == ShufflingTransaction.SHUFFLING_RECIPIENTS) {
						Attachment.ShufflingRecipients shufflingRecipients = (Attachment.ShufflingRecipients) transaction
								.getAttachment();
	...
	public long getEffectiveBalanceNXT(int height) {
		...
		try {
			...
			return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0
	...
	void addToUnconfirmedAssetBalanceQNT(LedgerEvent event, long eventId, long assetId, long quantityQNT) {
		...
		if (AccountLedger.mustLogEntry(this.id, true)) {
			AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id, LedgerHolding.UNCONFIRMED_ASSET_BALANCE,
					assetId, quantityQNT, unconfirmedAssetBalance));
		}
	}
	...
	void addToUnconfirmedCurrencyUnits(LedgerEvent event, long eventId, long currencyId, long units) {
		...
		AccountCurrency accountCurrency = accountCurrencyTable
				.get(accountCurrencyDbKeyFactory.newKey(this.id, currencyId));
		...
		if (AccountLedger.mustLogEntry(this.id, true)) {
			AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id, LedgerHolding.UNCONFIRMED_CURRENCY_BALANCE,
					currencyId, units, unconfirmedCurrencyUnits));
		}
	}
	...
	void payDividends(final long transactionId, Attachment.ColoredCoinsDividendPayment attachment) {
		...
		for (final AccountAsset accountAsset : accountAssets) {
			if (accountAsset.getAccountId() != this.id && accountAsset.getQuantityQNT() != 0) {
				...
				Account.getAccount(accountAsset.getAccountId()).addToBalanceAndUnconfirmedBalanceNQT(
						LedgerEvent.ASSET_DIVIDEND_PAYMENT, transactionId, dividend);
